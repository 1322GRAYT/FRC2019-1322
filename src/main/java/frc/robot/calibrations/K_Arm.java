...
public class K_Arm {
    ...
    public static final List<PositionData> BALL_POS_DATA = ALL_POS_DATA.stream()
            .filter(p -> p.gpType == GamePieces.Cargo || p.gpType == GamePieces.Both).collect(Collectors.toList());
    public static final List<PositionData> PANEL_POS_DATA = ALL_POS_DATA.stream()
            .filter(p -> p.gpType == GamePieces.HatchPanel || p.gpType == GamePieces.Both).collect(Collectors.toList());

    public final static PositionData[] ARM_POS_DATA = { new PositionData(0, "Floor Pickup", "Ball"),
            new PositionData(20500, "Rocket Level 1", "Panel"), new PositionData(98000, "Rocket Level 1", "Ball"),
            new PositionData(154500, "Rocket Level 2 / Cargo Ship", "Panel"),
            new PositionData(229000, "Rocket Level 2", "Ball"), new PositionData(297000, "Rocket Level 3", "Panel"),
